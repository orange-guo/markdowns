			...
			Feats: [
				{
					...
					Issues: [
						{
							Issue: "[user][api]新增用户api定义",
							ManDays: 0.5
						},
						{
							Issue: "[user][api]新增用户api实现",
							ManDays: 0.5
						}
					]
				},
				{
					...
					Issues: [
						{
							Issue: "[user][api]修改用户api定义",
							ManDays: 0.5
						},
						{
							Issue: "[user][api]修改用户api实现",
							ManDays: 0.5
						}
					]
				},
				{
					...
					Issues: [
						{
							Issue: "[user][api]删除用户api定义",
							ManDays: 0.5
						},
						{
							Issue: "[user][api]删除用户api实现",
							ManDays: 0.5
						}
					]
				},
				{
					...
					Issues: [
						{
							Issue: "[user][api]用户列表api定义",
							ManDays: 0.5
						},
						{
							Issue: "[user][api]用户列表api实现",
							ManDays: 0.5
						}
					]
				}
			...
			Feats: [
				...
				},
				{
					...
					Issues: [
						...
						{
							Issue: "[cli][job]terminate",
							ManDays: 0.5
						},
						{
							Issue: "[cli][job]create-image",
							ManDays: 0.5
						},
						{
							Issue: "[cli][job]list",
...
export function PrintFeat({Feat}) {
	...
	return (
		<div>
			...
			{Feat.Issues.map(Issue => <PrintIssue Issue={concat(Feat, Issue)}></PrintIssue>)}
			<br></br>
		</div>
	)
}
...
export function PrintMod({Mod}) {
	return (
		<div>
			...
			{Mod.Feats.map(Feat => <PrintFeat Feat={concat(Mod, Feat)}></PrintFeat>)}
			<hr></hr>
		</div>
	)
}
...
export function PrintProject({Project}) {
	return (
		<div>
			...
			{Project.Mods.map(Mod => <PrintMod Mod={concat(Project, Mod)}></PrintMod>)}
		</div>
	);
}

<PrintProject Project={Project}></PrintProject>
